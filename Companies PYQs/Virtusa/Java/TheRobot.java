...
public class TheRobot {
    ...
    public static int findOptimalTime(int N, int X, int[] S) {
        int singleModeTime = 0;
        int maxLength = 0;

        for (int i = 0; i < N; i++) {
            singleModeTime += Math.ceil((double) S[i] / X);
            maxLength = Math.max(maxLength, S[i]);
        }

        int dynamicModeTime = maxLength;

        return Math.min(singleModeTime, dynamicModeTime);
    }

    public static void main(String[] args) {
        System.out.println(findOptimalTime(5, 4, new int[]{2, 2, 4, 1, 1})); // Output: 4
        System.out.println(findOptimalTime(3, 10, new int[]{7, 8, 9}));      // Output: 3
    }
}
