...
public class Program2 {
    public static void main(String[] args) {
        ...
        String result = findClosestPair(list1, list2, targetSum);
        
        // Print the result
        System.out.println(result);
    }
    
    private static String findClosestPair(int[] list1, int[] list2, int target) {
        int left = 0;
        ...
        while (left < list1.length && right >= 0) {
            ...
            if (Math.abs(sum - target) < Math.abs(closestSum - target)) {
                closestSum = sum;
                closestPair = list1[left] + "," + list2[right];
            }
