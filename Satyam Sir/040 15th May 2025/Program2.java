...
public class Program2 {
    public static void main(String[] args) {
        ...
        System.out.println(correctWords(str1, str2));
    }

    private static List<String> correctWords(String[] str1, String[] str2) {
        Set<String> st = new HashSet<>();
        Map<String, String> mp1 = new HashMap<>();
        Map<String, String> mp2 = new HashMap<>();
        for (String word : str1) {
            st.add(word);
            String strr1 = word.toLowerCase();
            mp1.putIfAbsent(strr1, word);
            String strr2 = vowelMask(strr1);
            mp2.putIfAbsent(strr2, word);
        }
        ...
        for (String word : str2) {
            if (st.contains(word)) {
                ...
                if (mp1.containsKey(lower))
                    res.add(mp1.get(lower));
                else {
                    String mas = vowelMask(lower);
                    if (mp2.containsKey(mas))
                        res.add(mp2.get(mas));
                    else
                        res.add("");
                }
    ...
    private static String vowelMask(String word) {
