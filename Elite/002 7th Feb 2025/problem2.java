...
public class problem2 {
    static int solve(HashMap<Integer, Integer> mp, int f, int x) {
        PriorityQueue<int[]> ps = new PriorityQueue<>((a, b) -> (b[0] - a[0]));
        for (Integer k : mp.keySet()) {
            int score = ((int) Math.pow(k, f)) * mp.get(k);
            ps.add(new int[] { score, k });
        }
        int sol = 0;
        for (int i = 0; i < x; i++) {
            int temp[] = ps.poll();
