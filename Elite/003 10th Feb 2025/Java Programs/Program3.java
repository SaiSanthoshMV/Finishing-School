...
public class Program3{
    public static void main(String[] args){
        ...
        Deque<Integer> q2 = new ArrayDeque<>();
         while(k < n){
            while(!q1.isEmpty() && q1.peekLast() < arr[k]) q1.pollLast();
            q1.addLast(arr[k]);
            while(!q2.isEmpty() && q2.peekLast() > arr[k]) q2.pollLast();
            q2.addLast(arr[k]);
            if(k-m+1 == r){
                int vol = q1.peekFirst() - q2.peekFirst();
                if(vol > ans) ans = vol;
                if(q1.peekFirst() == arr[m]) q1.pollFirst();
                if(q2.peekFirst() == arr[m]) q2.pollFirst();
                m++;
            }
            k++;
        }
